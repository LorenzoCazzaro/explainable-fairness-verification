...
public:
    Counter(size_t d=0) {
        /* d = num of labels */
        this->d = d;
        /* se d > 0 ci sono label da trovare */
        state = (0==d);
    }

    bool add(float l) {
        v.insert(l);
        /* se v ha tante label quante sono quelle in totale allora state è true */
        state = (v.size()==d);
        return state;
    }
...
/*
UNIONE
SymbolicAttack operator+(const SymbolicAttack& l, const SymbolicAttack& r) {
    if (l.get_paid() == r.get_paid()) {
        HyperRectangle pre = l.get_pre() + r.get_pre();
        HyperRectangle post;
        if (!pre.is_empty())
            post = l.get_post() + r.get_post();

        return SymbolicAttack(pre, post, l.get_paid());
    }
   
    return SymbolicAttack(true);
}
*/
...
bool check_in(std::vector<float>& ist, std::set<SymbolicAttack>& U) {
    for (const SymbolicAttack& u : U)
        if(ist > u.get_pre())
            return true;
    return false;
...
/* TODO, sistemare questo casino molto brutto */
class Ensemble;
...
/* TODO RIMUOVERE TUTTI GLI * INUTILI */
class Ensemble {
...
public:
    ...
    float robustness(VV_f& data, V_f& labels, std::set<SymbolicAttack>& U) {
        float count = 0;

        for (size_t i = 0; i < data.size(); ++i) {
            float pred = this->predict(data[i]);
            if (pred == labels[i]) {
                bool robust = !check_in2(data[i], U, *this, labels[i]);
                if (robust)
                    count++;
            }
        }
    ...
    float resilience(VV_f& data, V_f& labels, std::set<SymbolicAttack>& U, HyperRectangle& N) {
        float count = 0;
        for (size_t i = 0; i < data.size(); ++i) {
            float pred = this->predict(data[i]);
            if (pred == labels[i]) {
    ...
    float bounding(VV_f& data, V_f& labels, std::set<SymbolicAttack>& U, Attacker& A) {
        float count = 0;
        for (size_t i = 0; i < data.size(); ++i) {
            float pred = this->predict(data[i]);
            if (pred == labels[i]) {
                HyperRectangle B_data = data[i] + A.get_atk();
                bool agreement = score_ensemble(B_data, labels[i]);
                if (agreement)
                    count++;
            }
        }
    ...
    float bounding_res(VV_f& data, V_f& labels, Attacker& A) {
        float count = 0;

        std::vector<Interval> intvs;
        for (size_t i=0; i < A.get_dim(); ++i)
            intvs.emplace_back(-0.01, 0.01, 0, 0);

        HyperRectangle base(intvs);
    ...
    float bounding_robustness(VV_f& data, V_f& labels, std::set<SymbolicAttack>& U, Attacker& A) {
        float count = 0;
        Interval base(0, 0, 0, 0);
        std::vector<Interval> intvs;
        ...
        for (size_t i = 0; i < data.size(); ++i) {
            ...
            if (pred == labels[i]) {
                for (float v : data[i])
                    intvs.emplace_back(v + base);
                HyperRectangle x(intvs);
                bool robust = check_inter(x, U);
                intvs.clear();
                if (robust)
                    count++;
                else {
                    HyperRectangle B_data = data[i] + A.get_atk();
                    bool res = score_ensemble(B_data, labels[i]);
                    if(res)
                        count++;
                }
            }
    ...
    float bounding_and_robustness(VV_f& data, V_f& labels, std::set<SymbolicAttack>& U, Attacker& A) {
        float robust_bounding_count = 0;
        ...
        return robust_bounding_count / data.size();
    }

    void fast_statistics(VV_f& data, V_f& labels, Attacker& A, std::set<SymbolicAttack>& U, HyperRectangle& N) {
        float robust_count = 0;
        float resilient_count = 0;
        float bounding_count = 0;
        float robust_bounding_count = 0;
        ...
        std::cout << "robustness: " << robust_count / data.size() << std::endl;
        std::cout << "bounding: " << bounding_count / data.size() << std::endl;
        std::cout << "bounding & robustness: " << robust_bounding_count / data.size() << std::endl;
        std::cout << "resilience: " << resilient_count / data.size() << std::endl;
    }

    void fast_statistics2(VV_f& data, V_f& labels, Attacker& A, std::set<SymbolicAttack>& U, HyperRectangle& N, float* robustness, float* bounding_robustness, float* bounding_and_robustness, float* resilience) {
        float robust_count = 0;
        float resilient_count = 0;
        float bounding_count = 0;
        float robust_bounding_count = 0;
        ...
        *robustness = robust_count / data.size();
        *bounding_robustness = bounding_count / data.size();
        *bounding_and_robustness = robust_bounding_count / data.size();
        *resilience = resilient_count / data.size();
    }

    //utilizzata per computare le statistics con U e U ended separati nella analyze ensemble optimized
    void fast_statistics2_Uended(VV_f& data, V_f& labels, Attacker& A, std::set<SymbolicAttack>& U, std::set<SymbolicAttack>& U_ended, HyperRectangle& N, float* robustness, float* bounding_robustness, float* bounding_and_robustness, float* resilience) {
        float robust_count = 0;
        float resilient_count = 0;
        float bounding_count = 0;
        float robust_bounding_count = 0;
        ...
        for (size_t i = 0; i < data.size(); ++i) {
            ...
            if (pred == labels[i]) {
                ...
                if (robust) {
                    ...
                    bool robust = !check_in2(data[i], U_ended, *this, labels[i]);
                    if (robust) {
                        robust_count++;
                        robust_bounding_count++;
                        ...
                        /* check resilience */
                        HyperRectangle N_data = data[i] + N;
                        bool resilient = check_inter(N_data, U_ended);
                        if (resilient)
                            resilient_count++;
                    }
                }
    ...
    /* STAMPA DOVE BOUNDING È ROBUSTA E NOSTRO NO */
    void rob_vs_bnd(VV_f& data, V_f& labels, Attacker& A, std::set<SymbolicAttack>& U, HyperRectangle& N) {
        for (size_t i = 0; i < data.size(); ++i) {
            float pred = this->predict(data[i]);
            if (pred == labels[i]) {
                /* check bounding */
                HyperRectangle B_data = data[i] + A.get_atk();
                bool agreement = score_ensemble(B_data, labels[i]);
                bool false_positive = check_inter(B_data, U);

                if (agreement) {
                    /* check robustness */
                    for (const SymbolicAttack& u : U)
                        if(data[i] > u.get_pre()) {
                            std::cout << "INTERSECA" << std::endl;
                            print_ist(data[i]);
                            B_data.print();
                            u.print();
                            std::cout << "bound robust?" << false_positive << std::endl;
                            std::cout << " " << std::endl;
                        }
                }                
            }
